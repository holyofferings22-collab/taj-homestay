...
export function HeroStage({
  ...
  return (
    /* From `md` the stage fills the screen and the name and the card float
       over its foot. On a phone the photographs are landscape frames and a
       full-screen crop showed a sliver of each, so there the photograph is
       a block of the width's own proportion (4:5, 4:3 from `sm`) that
       scales with the screen, the name sits at its foot, and the card sits
       beneath it on the page rather than over it. */
    <div ref={scope} className="relative md:absolute md:inset-0 md:bg-ash">
      <div className="relative aspect-[4/5] overflow-hidden bg-ash sm:aspect-[4/3] md:absolute md:inset-0 md:aspect-auto">
        {/* The photographs. */}
          ...
          {frames.map((frame, i) => (
            ...
            >
              {/* Every frame loads eagerly. Lazy loading looks like the right
                  ...
                  the rest are eager at low priority, so they arrive during
                  the idle time after first paint without competing with it. */}
              <Image
                src={frame.src}
                alt={i === 0 ? frame.alt : ''}
                fill
        ...
        {/* Shade at the top so the header's type holds, and along the foot
            so the name and the bar do. Nothing across the middle. */}
        <div
          aria-hidden="true"
        ...
        {/* The name and the way into the gallery, one row at the foot of
            the photograph, never wrapped: the name at the left, the gallery
            button at the right on the same line. Below `sm` the button is
            the thumbnail alone, so the name keeps room to break into two
            lines beside it. From `md` the row sits above the floating card,
            and its right padding keeps the button clear of the live-support
            launcher in the same corner; on a phone the launcher steps aside
            over the hero. */}
        <div className="absolute inset-x-0 bottom-[clamp(16px,4vw,28px)] z-[2] px-[var(--gutter)] md:bottom-[calc(clamp(22px,5vh,54px)+61px+clamp(18px,3.5vh,40px))]">
      ...
      {/* Availability. On a phone it follows the photograph on the page's
          own ground; from `md` it floats clear of the foot, so it reads as
          sitting in front of the photograph rather than built into it. */}
      <div className="bg-mist px-[var(--gutter)] py-[clamp(16px,4vw,24px)] md:absolute md:inset-x-0 md:bottom-[clamp(22px,5vh,54px)] md:z-[3] md:bg-transparent md:py-0">
